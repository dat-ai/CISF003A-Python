...
class Board(object):
    ...
    def __init__(self, grid_size=3, box_size=200, border=20, line_width=5):
        ...
        # Create pygameTool (add button ,add sound easier)
        self.pgTool = UtilPyGame(self, self.surface_size)
        self.pgTool.add_sound(M_START)
    ...
    def display_start_screen(self):
        ...
        try:
            self.pgTool.add_text_to_screen("Welcome to TicTacToe Game", WHITE, "large", -self.surface_size * 0.05, LIGHT_GREEN)
            self.pgTool.add_text_to_screen("CIS F003A - Fall 2016", WHITE, "small", self.surface_size * 0.05, LIGHT_GREEN)
            self.pgTool.add_button(button_pos, DARK_GRAY, "Click me to continue", WHITE)
        finally:
            pass
        ...
        while intro:
            ...
            if self.is_hover(button_pos):
                ...
                if click[0] == 1:
                    break
                self.pgTool.add_button(button_pos, WHITE, "Click me to continue", BLACK)
    ...
    def setup(self):
        ...
        try:
            self.pgTool.add_text_to_screen("Turn: Player %s" % self.turn, BLACK, "small", int(self.surface_size/2.2), DARK_GRAY)
        except Exception as e:
            print("Pygame.init is not running yet. %s" % str(e))
        finally:
            pass
    ...
    def play_turn(self, box):
        ...
        if box.state != 0:
            return
        if self.turn == 1:
            self.pgTool.add_sound(M_P1)
            box.mark_x()
            box.state = 1
            self.turn = 2
        elif self.turn == 2:
            self.pgTool.add_sound(M_P2)
            box.mark_o()
            box.state = 2
            self.turn = 1
        return
    ...
    def display_game_over(self, winner):
        ...
        x = self.surface_size
        if winner:
            self.pgTool.add_sound(M_WINNER)
            text = 'Player %s won!' % winner
            self.result[winner-1] += 1
        else:
            self.pgTool.add_sound(M_DRAW)
            text = 'Draw Game!'
        # Display result
        self.surface.fill(DARK_GRAY)
        self.pgTool.add_text_to_screen(text, BLACK, 60, 0, DARK_GRAY)
        # Restart or quit
        restart_button = (x / 2 - x * 0.40, x - x*0.2, 150, 100)
        quit_button = (x / 2 + x * 0.05, x - x*0.2, 150, 100)
        self.pgTool.add_text_to_screen("Player 1: %s" % str(self.result[0]), LIGHT_GREEN, "small", -int(x/2.2), DARK_GRAY, -int(x*0.25))
        self.pgTool.add_text_to_screen("Player 2: %s" % str(self.result[1]), LIGHT_GREEN, "small", -int(x/2.2), DARK_GRAY, int(x*0.25))
        end = True
        while end:
            self.pgTool.add_button(restart_button, DARK_GRAY, "Restart", LIGHT_GREEN)
            self.pgTool.add_button(quit_button, DARK_GRAY, "Quit", LIGHT_GREEN)
            ...
            if self.is_hover(restart_button):
                click = pygame.mouse.get_pressed()
                self.pgTool.add_button(restart_button, DARK_GRAY, "Restart", WHITE)
                if click[0] == 1:
                    break
            # If user enter quit button, quit the program
            if self.is_hover(quit_button):
                click = pygame.mouse.get_pressed()
                self.pgTool.add_button(quit_button, DARK_GRAY, "Quit", WHITE)
                if click[0] == 1:
                    pygame.quit()
                    sys.exit()
            pygame.display.update()
...
class UtilPyGame:
    ...
    def add_text_to_screen(self, message, color, size="small", y_offset=0, rect_color=(255, 255, 255), x_offset=0):
        """
        ...
        rect.center = (self.surface_size / 2 + x_offset, self.surface_size / 2 + y_offset)
        self.board.surface.blit(text, rect)
    ...
    def add_button(self, size, color, text, text_color):
        ...
        if text_surf is not None:
            ...
            self.board.surface.blit(text_surf, text_rect)
...
class Box(object):
    ...
    def mark_x(self):
        ...
        s = self.line_width
        r = self.radius
        pygame.draw.line(self.board.surface, BLACK, (self.rect.centerx - r, self.rect.centery - r),
                         (self.rect.centerx + r, self.rect.centery + r), s)
        pygame.draw.line(self.board.surface, BLACK, (self.rect.centerx - r, self.rect.centery + r),
                         (self.rect.centerx + r, self.rect.centery - r), s)
    ...
    def mark_o(self):
        ...
        s = self.line_width
        r = self.radius
        pygame.draw.circle(self.board.surface, LIGHT_YELLOW, (int(self.rect.centerx), int(self.rect.centery)), int(r),
                           int(s))
